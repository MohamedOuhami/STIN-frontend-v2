  ...
  const columns = [
    {
      ...
      headerName: 'Element',
      width: 300,
      renderCell: (params) => params.row.element.name
    },
    {
      field: 'salle',
      headerName: 'Salle',
    ...
    },
    {
      ...
      width: 300,
      renderCell: (params) => params.row.professor ? params.row.professor.firstName + " " + params.row.professor.lastName : "Not assigned"
    },
    {
      field: 'timeTable',
      headerName: 'Emploi du temps',
      width: 300,
      renderCell: (params) => params.row.timeTable ? params.row.timeTable.filiereSemestre.filiere.name + " " + params.row.timeTable.filiereSemestre.semestre.count + " " + params.row.timeTable.section.name : "Not assigned"
    },
    {
      ...
      renderCell: (params) => (
        <div> {/* Wrapped buttons inside a div */}
          <Button onClick={() => handleAssignProfessor(params.row.id)} variant="contained">Assign professor</Button>
        </div>
      )
    }
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8080/seances', {
