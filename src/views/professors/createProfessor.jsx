  ...
  const handleCreatePrepaStudent = async () => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:8080/prepaStudents',
        { lastName, firstName, email, username, dateNaissance, cin, cne, telephone, lieuNaissance, semestre },
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
              ...
              >
                {semestres &&
                  semestres.map((semestre) => {
                    ...
                    return null; // Ignorer le rendu de cette option
                  })}
              </Form.Control>
            </Form.Group>
          </Col>
